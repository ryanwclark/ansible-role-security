...
[![CI](https://github.com/ryanwclark/ansible-role-security/workflows/CI/badge.svg?event=push)](https://github.com/ryanwclark/ansible-role-security/actions?query=workflow%3ACI)
...
There are a few other things you may or may not want to do (which are not included in this role) to make sure your servers are more secure, like:
  ...
  - Have a well-configured firewall (check out the `ryanwclark.firewall` role on Ansible Galaxy for a flexible example)
...
On RedHat/CentOS systems, make sure you have the EPEL repository installed (you can include the `ryanwclark.repo-epel` role to get it installed).
